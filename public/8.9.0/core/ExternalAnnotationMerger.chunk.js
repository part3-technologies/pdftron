			...
			471: function (Ba, va, r) {
				...
				Ba = (function () {
					...
					da.prototype.merge = function (aa, y, x) {
						...
						b &&
							(this.eia(aa, b.Jn),
							this.gia(aa, b.z1, y.Qw),
							this.fia(aa, b.page, x),
							this.cia(aa, b.MT));
						b = this.aa.Nb();
						if (x === b - 1) {
							var e = y.Qw;
					...
					da.prototype.bia = function (aa, y) {
						null !== y &&
							((aa = this.bw(aa)), this.$q(aa, 'calculation-order', y));
					};
					da.prototype.dia = function (aa, y) {
						null !== y &&
							((aa = this.bw(aa)), this.$q(aa, 'document-actions', y));
					};
					...
					da.prototype.cia = function (aa, y) {
						Object.keys(y).forEach(function (x) {
							(x = aa.querySelector('annots [name="' + x + '"]')) &&
					...
					da.prototype.HM = function (aa, y, x, h) {
						if (x === y.length)
							...
							var b = y[x];
							this.aw(aa, '[name="' + b + '"]', 'field').setAttribute(
								'name',
								b
							);
								...
								for (
									...
									(da = na.element.children[fa]),
										'value' === da.tagName
											...
											: da.children &&
											  ma.push({
													name: na.name + '.' + da.getAttribute('name'),
													element: da,
											  });
						}
					};
