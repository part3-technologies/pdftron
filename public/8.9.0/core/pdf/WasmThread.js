	...
	$jscomp.polyfill = function (p, l, r, n) {
		l &&
			($jscomp.ISOLATE_POLYFILLS
				? $jscomp.polyfillIsolated(p, l, r, n)
				: $jscomp.polyfillUnisolated(p, l, r, n));
	};
				...
				return t.done
					...
					: new k(function (v, w) {
							...
							do
								y.push(void 0),
									z++,
									r(t.value).callWhenSettled_(E(y.length - 1), w),
									(t = m.next());
							while (!t.done);
					  });
			};
			...
			(function (n) {
				...
				var m = q.Module,
					...
					w = function (b) {
						...
						b in q.AsyncFSHack.readCalls ||
							(q.AsyncFSHack.readCalls[b] = new g(
								q.AsyncFSHack.read_total,
								q.AsyncFSHack.q
							));
					},
					E = function (b) {
						...
						c.fs_read_counter >= c.fs_read_total &&
							(c.needCallback
								...
								: (q.AsyncFSHack.readingAsyncFS = 0),
				...
				y.prototype = {
						...
						if (h) {
							...
							if (this.customHeaders)
								for (var d in this.customHeaders)
									e.setRequestHeader(d, this.customHeaders[d]);
							e.send();
							var a = this;
							e.onload = function () {
					...
					readFromCache: function (b, c, h) {
						...
						c = c > h ? h : c;
						this.chunkMap[a]
							? ((f = this.chunkMap[a].subarray(f, f + c)),
							  b.set(f),
							  this.lruUpdate(a))
							: this.hadChunk(a)
							? e.push([a, f, c, d])
						...
						for (h -= c; 0 < h; )
							(d += c),
								...
								this.chunkMap[a]
									? ((f = this.chunkMap[a].subarray(0, c)),
									  b.set(f, d),
									  this.lruUpdate(a))
									: this.hadChunk(a)
									? e.push([a, 0, c, d])
						...
						(a = f.value),
							'url' === f.type
								? ((a = Object(k.b)(
								  ...
								  (f.value.docDevId = a))
								: Object(k.d)(a) &&
								  ((a = Object(k.a)(a)), (C[u] = a), (f.value = a));
							...
							? ((u = f.docId),
							  u in B &&
									...
									a in q.Module.docs &&
										((a = q.Module.docs[a]),
										'chunkStorage' in a &&
											((f = q.Module.GetRequiredChunkOffsetArray(
												u,
												f.pageIndex + 1
											)),
			...
			H.prototype = {
				...
				handleMessage: function (d) {
					var a = d.callbackId,
						f = d.data;
					if (a in C)
						f && f.docId ? (B[f.docId] = C[a]) : Object(k.c)(C[a]), delete C[a];
					else if (a in A) {
