	...
	(window.wpCoreControlsBundle = window.wpCoreControlsBundle || []).push([
		[4],
		{
			...
			86: function (Ba, va, r) {
							...
							switch (ub.label) {
								...
								case 5:
									return (
										...
										Za ||
											Na ||
											(ba(nb, sb.galleys, xa, Na), ba(nb, sb.objects, xa, Na)),
										[2]
									);
							}
						});
				...
				function fa(mb, nb) {
					if (null === mb || void 0 === mb ? 0 : mb.includes('bold'))
						nb = '<strong>' + nb + '</strong>';
					if (null === mb || void 0 === mb ? 0 : mb.includes('italic'))
						nb = '<em>' + nb + '</em>';
					if (
						(null === mb || void 0 === mb ? 0 : mb.includes('underline:1;')) ||
						(null === mb || void 0 === mb
							? 0
							: mb.includes('text-decoration: underline'))
					)
						nb = '<u>' + nb + '</u>';
					return nb;
				}
				function da(mb) {
					if (
						['STRONG', 'EM', 'U'].includes(mb.tagName) &&
						null !== mb.getAttribute('style')
					) {
						...
						'SPAN' === mb.tagName && null !== mb.getAttribute('style')
							? ((rb = mb.style.color),
							  (rb = ca(rb)),
							  mb.setAttribute('style', 'color:' + rb))
				...
				function b(mb) {
					...
					rb.querySelector('parsererror') &&
						(rb = nb.parseFromString('<Root>' + mb + '</Root>', 'text/xml'));
				...
				function ka(mb, nb, rb, sb, xa) {
					...
					for (
						sb = rb.documentElement.querySelectorAll('p').length;
						sb < Na.length;
						sb++
					) {
					...
					nb.forEach(function (cb) {
						if (cb instanceof z) {
							...
							(jb = Na.mo(mb, cb.galleyBox.leftVal(), cb.galleyBox.topVal())),
								...
								(lb = Na.mo(
									mb,
									cb.galleyBox.rightVal(),
									cb.galleyBox.bottomVal()
								)),
								(jb = lb.x),
								(lb = lb.y);
						else return;
						...
						return Object(za.d)(this, function (Cb) {
							switch (Cb.label) {
								...
								case 2:
									return (
										(Za = Cb.ca()),
										Wa.postMessage({ cmd: 'initialiseInfixServer', l: Za }),
										[3, 10]
									);
								case 3:
									return (
				...
				function la(mb, nb) {
					...
					sb = sb.innerHTML;
					var Na = Array.prototype.slice.call(
						rb.getElementsByTagName('galleys')
					)[0];
				...
				function pa(mb, nb, rb, sb) {
					...
					Array.prototype.slice
						.call(sb.getElementsByTagName('BBox'))
						.forEach(function (jb) {
							if ('CropBox' === jb.getAttribute('Name')) {
								var zb = parseFloat(
										jb.getElementsByTagName('Top').item(0).innerHTML
									),
									eb = parseFloat(
										jb.getElementsByTagName('Bottom').item(0).innerHTML
									),
									lb = parseFloat(
										jb.getElementsByTagName('Left').item(0).innerHTML
									);
								jb = parseFloat(
									jb.getElementsByTagName('Right').item(0).innerHTML
								);
					...
					var Za = [];
					Array.prototype.slice
						.call(sb.getElementsByTagName('STORY'))
						.forEach(function (jb) {
							var zb = jb.getAttribute('galley_ids'),
								eb = Array.prototype.slice.call(
									jb.getElementsByTagName('g')
								)[0],
							...
							eb = eb.innerHTML;
							var tb = Array.prototype.slice.call(
								jb.getElementsByTagName('galleys')
							)[0];
					...
					Na = new DOMParser();
					var cb = [];
					nb = Na.parseFromString(rb, 'text/xml').getElementsByTagName(
						'Object'
					);
					Array.prototype.slice.call(nb).forEach(function (jb) {
						...
						jb.forEach(function (ib) {
							...
							'TL' === ub
								? ((lb = ra(ib.getAttribute('Y'), lb)),
								  (Cb = ra(ib.getAttribute('X'), Cb)))
								: 'TR' === ub
								? ((lb = ra(ib.getAttribute('Y'), lb)),
								  (Nb = ra(ib.getAttribute('X'), Nb)))
								: 'BR' === ub
								? ((tb = ra(ib.getAttribute('Y'), tb)),
								  (Nb = ra(ib.getAttribute('X'), Nb)))
								: 'BL' === ub &&
								  ((tb = ra(ib.getAttribute('Y'), tb)),
								  (Cb = ra(ib.getAttribute('X'), Cb)));
					...
					nb = Array.prototype.slice
						.call(sb.getElementsByTagName('DOC'))[0]
						.getAttribute('id');
					...
					if (null != nb) {
						...
						Oa[mb].forEach(function (cb) {
							if (cb.id === Za) {
								...
								jb += '</Contents>';
								var zb = Array.prototype.slice.call(
									sb.getElementsByTagName('NewPara')
								);
								jb = new DOMParser().parseFromString(jb, 'text/xml');
								var eb = Array.prototype.slice.call(
									jb.getElementsByTagName('p')
								);
				...
				function wa(mb) {
					...
					Array.prototype.slice.call(mb).forEach(function (rb) {
						...
						var xa = [];
						Array.prototype.slice
							.call(rb.getElementsByTagName('Variation'))
							.forEach(function (Na) {
								Na = Na.innerHTML;
								xa.push(Na);
								if (Na.includes('Regular') || Na === sb.replace(/\s+/g, ''))
									nb[sb].hasRegular = !0;
				...
				va['default'] = {
					...
					Hpa: function (mb) {
						...
						rb.forEach(function (Za) {
							if (xa || Na) {
								Za.style.textDecoration = Za.style.textDecoration.replace(
									'underline',
									''
								);
